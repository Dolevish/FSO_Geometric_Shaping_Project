function run_Joint_SA_fmincon_sweep_parallel_M8_SNR15()
...
%
% Output folder:
%   results_joint_SA_fmincon_sweep_parallel_m8_snr15/
%
% Main outputs:
...
%   Joint_SA_fmincon_sweep_gain_by_turbulence_sigma_*.png
    ...
    cfg.nRandomStarts = 30;       % Increase to 20+ for stronger global search.
    ...
    % Parallelization
    cfg.useParallelCases = false; % Single case script: keep cases serial.
    cfg.useParallelStarts = true;  % Parallel over random feasible starts inside the case.
    ...
    % Output
    outDir = fullfile(pwd, 'results_joint_SA_fmincon_sweep_parallel_m8_snr15');
    if ~exist(outDir, 'dir')
        mkdir(outDir);
    end
    ...
    fprintf('    parallel starts   = %d\n', cfg.useParallelStarts);

    %% ============================================================
    %  3. Start parallel pool
    % =============================================================
    if cfg.useParallelCases || cfg.useParallelStarts
        ppool = gcp('nocreate');
        if isempty(ppool)
        ...
        fprintf('\n[3] Running without a parallel pool because both case/start parallel flags are false\n');
    end
...
function res = run_one_case(caseIdx, sigma_X_sq, SNR_dB, cfg, outDir)
    ...
    if cfg.useParallelStarts
        parfor s = 1:cfg.nRandomStarts
            runList(s) = run_one_random_start(s, caseIdx, params, cfg, opts, y_grid);
        end
    else
...
function sr = run_one_random_start(s, caseIdx, params, cfg, optsTemplate, y_grid)
    M = cfg.M;
    P_avg = cfg.P_avg;

    sr = empty_start_result(M);
    ...
    try
        ...
        if cfg.useJointSA
            ...
            out_sa = simulated_annealing_joint_gs_ps(x0, p0, params, saOpts);
            x_sa = out_sa.x_best(:);
            p_sa = out_sa.p_best(:);
        ...
        opts_s = optsTemplate;
        opts_s.seed = seedStart + 17;
        out_s = joint_fmincon_refine_gs_ps(x_sa, p_sa, params, opts_s);

        sr.startIndex = s;
        sr.seed = seedStart;
        sr.x0 = x0;
        sr.p0 = p0;
        sr.ami0_fast = ami0_fast;
        sr.ami0_val = ami0_val;
        sr.x_sa = x_sa;
        sr.p_sa = p_sa;
        sr.ami_sa_fast = ami_sa_fast;
        sr.ami_sa_val = ami_sa_val;
        sr.saRuntime = saRuntime;
        sr.saAcceptRate = saAcceptRate;
        sr.x_best = out_s.x_best(:);
        sr.p_best = out_s.p_best(:);
        sr.ami_fast = out_s.ami_fast;
        sr.ami_val = out_s.ami_val;
        sr.exitflag = out_s.exitflag;
        sr.ok = true;
        sr.errorMessage = '';
    catch ME
        sr.startIndex = s;
